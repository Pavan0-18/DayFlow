  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Daily Patrol Log</h1>
      ...
      <GlassPanel variant="default" className="p-3">
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="icon" onClick={() => setDate((d) => subDays(d, 1))} className="text-muted-foreground hover:text-foreground">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <span className="text-base font-semibold text-foreground">{format(date, "MMMM d, yyyy")}</span>
          <Button variant="ghost" size="icon" onClick={() => setDate((d) => addDays(d, 1))} disabled={!canGoForward} className="text-muted-foreground hover:text-foreground disabled:opacity-30">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <GlassPanel variant="holographic" className="p-6 flex flex-col items-center">
            <ProgressRing percentage={report.completionRate} size={160}>
              <div className="text-center">
                <span className="text-2xl font-bold text-foreground">{report.completionRate}%</span>
          ...
          <GlassPanel variant="strong" className="p-6 space-y-4">
            <div className="flex justify-between border-b border-border/50 pb-2">
              ...
              <span className="text-sm font-semibold text-foreground">{report.completedTasks}</span>
            </div>
            <div className="flex justify-between border-b border-border/50 pb-2">
              <span className="text-sm text-muted-foreground">Total tasks</span>
              <span className="text-sm font-semibold text-foreground">{report.totalTasks}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Day score</span>
              <span className="text-sm font-semibold text-foreground">{report.dayScore}</span>
            </div>
          </GlassPanel>
        </div>
      ) : (
